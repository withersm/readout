...
char sprts[1][15] = {"/dev/ttyACM2"};        /* deault */
int nsprts = 1;
...
int main(int argc, char *argv[])
{
    ...
    // printf("argc = %d\n", argc);
    if( argc < 2 )   {
        /* REM argc count includes this program name, */
        /* so argc should be 2 or 3, and first arg is argv[1], not [0] */
        printf(
           "must have 1 or 2 args: outfilename [/dev/ttyXXX for Arduino]\n");
        exit(-1);
    }
    sscanf(argv[1], "%s", outfilename);
    if( argc >= 3 )  {
        sscanf(argv[2], "%s", sprts[0]);
        /* otherwise retains its default value */
    }
    printf("%s %s  %s \n", argv[0], outfilename, sprts[0]);
