...
def record_position():
	...
	x = s[0]
	y = s[1]
	log = '/home/matt/ali_drive_mnt/beam_mapper_log.txt'
...
def continuous_rotation(motor_number,direction='ccw'): #rotates the motor continuously (e.g., when using the polarizer)
	global command
	if direction == "ccw":
		step_num = -400
	elif direction == "cw":
	...
	if int(motor_number) == 1:
	#set speed, acceleration, and direction (- button;ccw rotation of turn table and looking into the motor)
		print('Rotating motor 1')
		command += f"U77,LM0,I1M{step_num},L0,U9" + ","
	elif int(motor_number) == 2:
		command += f"U77,LM0,I2M{step_num},L0,U9" + ","
		
	print(command)

def stop_continuous_rotation(motor_number): #not currently operational
	global command
	if motor_number == 1:
		command += "U9" + ","
	elif motor_number == 2:
...
if ser.isOpen():
	...
	while 1:
		...
		print('INFO, SPEED, POSITION, STEP, SET_HOME, GO_HOME, CONTINUOUS ROTATE, PROGRAM, VXM, EXIT\n')
		command = 'C '
		i = input(">> ")
		if i == 'exit':
			ser.close()
		...
		elif i == "continuous rotate" or i == "CONTINUOUS ROTATE":
			motor_number = input('Which motor [1] or [2]: ')
			direction = input("Which direction [ccw] or [cw]: ")
			continuous_rotation(motor_number,direction) #need to figure out how to send the next command
			#stop = input('Press enter to stop the motor.')
			#stop_continuous_rotation(motor_number)
		
		elif i == 'decelerate' or i == "decelerate": #not currently operational
			stop_continuous_rotation(1)
...
"""
		while True: # read timestamp and position into log file while motor is moving
			...
			while ser.inWaiting() > 0:
				record_position()
				status = bytearray()
				status.extend(ser.read(1))
			status = status.decode("utf-8")[:-1]
