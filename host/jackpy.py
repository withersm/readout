...
subscribe message Out[6]: {'type': 'subscribe', 'pattern': None, 
'channel': b'picard', 'data': 1}

  ******  REVISIONS *****
2022-07-25
     - Began overhaul of command <-> repsonse. Previously we only had a 
busy please wait approach
         however now we will be utilizing pub-sub to facilitate replys 
with data payloads.
         Write waveform for instance shall now reply back with the 
written baseband frequencies.
...
def wait_for_reply(redis_pubsub, cmd, max_timeout=15):
     ...
     We want to have smarter replies that can ferry data back from the 
RFSOC.

     Once a command is sent out, listen for a reply on the <cmd_reply> 
channel
     ...
     :param max_timeout: int :
         time in seconds to wait for the RFSOC to reply. If it fails in 
this time,
         this should indicate a failure of some kind has occured
     :param cmd: str :
         Command sent out
     :param redis_pubsub: redis.Redis.pubsub :
         reference to a Redis pubsub object that has already subscribed 
to relevant channels
     ...
     print("WARINNG: TIMED OUT WAITING FOR REPLY -->  def 
waitForReply(redisIF, cmd, maxTimeout = 15):")
...
def sweep(loSource, udp, f_center, freqs, N_steps=500, freq_step=0):
     ...
     f_center : float
         Frequency center
     freqs : float[]
         List of Baseband Frequencies returned from rfsocInterface.py's 
writeWaveform()
     udp : udpcap.udpcap object instance
         udp data capture utility. This is our bread and butter for 
taking data from ethernet
     N_steps : int
     ...
     def temp(lofreq):
         ...
         # Read values and trash initial read, suspecting linear delay 
is cause..
         Naccums = 50
         I, Q = [], []
...
def loSweep(loSource, udp, freqs=[], f_center=default_f_center, 
N_steps=500, freq_step=0, savefile='s21'):
     """
     vnaSweep: perform a stepped frequency sweep centered at f_center 
and save result as s21.npy file
     f_center: center frequency for sweep in [MHz]
     """
#    print(freqs)
     f, sweep_Z_f = sweep(loSource, udp, f_center, np.array(freqs)/2, 
N_steps=N_steps, freq_step=freq_step)
...
def write_fList(self,fList):
     ...
     success, self.current_waveform = wait_for_reply(self.p, 
"ulWaveform", max_timeout=10)
     if success:
     ...
     def main_opt(self):
         ...
         while 1:
             ...
             if conStatus:
                 ...
                 print('\033[0;31m' +
                       "\r\nCouldn't connect to redis-server double 
check it's running and the generalConfig is correct" +
                       '\033[0m')
             opt = menu(self.captions, self.__main_opts)
             if conStatus == False:
                 resp = input("Can't connect to redis server, do you 
want to continue anyway? [y/n]: ")
             ...
             if opt == 2:  # Write test comb
                 ...
                 os.system("clear")
                 if prompt == 'y':
                     print("Waiting for the RFSOC to finish writing the 
full comb")
                     write_fList(self,[])
                 else:
                     print("Waiting for the RFSOC to write single {} MHz 
Tone".format(
...
custom frequency list: \r\n{}".format(
                 ...
                 # separate values from config and remove ',' before 
converting to number and
             ...
             if opt == 5:
                 ...
                 if t <= 0:
                     ...
                     fname = self.__saveData + 
"kidpyCaptureData{0:%Y%m%d%H%M%S}.h5".format(datetime.datetime.now())
                     print(fname)
                     if t < 60:
             ...
             if opt == 6:  # Lo Sweep
                 # valon should be connected and differentiated as part 
of bringing kidpy up.
                 os.system("clear")
                 print("LO Sweep")
...
default_f_center)
             ...
             if opt == 8:  # ONR version of kidpy
               if onr_flag:
                 onr_loop = True
                 while onr_loop:
                     onr_caption = ['\n\t\033[95mKID-PY2 RFSoC Readout 
(ONR Version)\033[95m']
                     onr_options = ['Write Custom Tone List', 'Standard 
Calibration LO Sweep', 'Plot LO Sweep', 'Get Cooridinates','Set AZ 
Position', 'Set EL Position','Exit']
                     onr_opt = menu(onr_caption, onr_options)

                     if onr_opt == 0:  # write stored comb
                         os.system("clear")

                         #see if the user wants the default list or 
something different:
                         tone_file = input('What is the tone file you 
would like to load (default is params/Default_tone_list.npy)') \
                           or 'params/Default_tone_list.npy'
                         fList = np.ndarray.tolist(np.load(tone_file))
                         print("Waiting for the RFSOC to finish writing 
the custom frequency list")
                         write_fList(self,fList)

                     if onr_opt == 1: #Run standard calibration LO sweep

                         #see if the user wants to shift all the tones 
(e.g., due to change in loading)
                         tone_shift = input('How many kHz to shift the 
tones before the LO sweep (default is 0)') \
                           or 0
                         if tone_shift != 0:
                             lo_freq = 
valon5009.Synthesizer.get_frequency(self.valon,valon5009.SYNTH_B)
                             fList = 
np.ndarray.tolist(get_tone_list(self) + float(tone_shift)*1.e3 - 
lo_freq*1.e6)
                             print("Waiting for the RFSOC to finish 
writing the updated frequency list")
                             write_fList(self,fList)

                         #first the low resolution initial sweep
                         os.system("clear")
                         print("Taking initial low-resoluation sweep 
with df = 1 kHz and Deltaf = 100 kHz")
                         savefile = onrkidpy.get_filename(type='LO')
                         loSweep(self.valon, self.__udp, 
self.current_waveform, \
valon5009.Synthesizer.get_frequency(self.valon,valon5009.SYNTH_B), \
                                 N_steps=200, freq_step=0.001, 
savefile=savefile)

                         #then fit the resonances from that sweep
                         fit_f0, fit_qi, fit_qc = 
fit_lo.main(get_tone_list(self),quickPlot=True,printFlag=True)
                         adjust_tones = input('Manually adjust any of 
the fitted f0 values (y/n)?') or 'n'
                         if adjust_tones == 'y':
                             keepgoing = 'y'
                             while keepgoing == 'y':
                                 adjust_index = int(input('Index of tone 
to adjust:  '))
                                 freq_adjust = float(input('Frequency 
(in MHz):  '))
                                 fit_f0[adjust_index] = freq_adjust * 1.e6
                                 keepgoing = input('Adjust another tone 
(y/n)?') or 'n'
                         new_tone_list = fit_f0 - 
valon5009.Synthesizer.get_frequency(self.valon,valon5009.SYNTH_B)*1.e6

#                        pdb.set_trace()


                     if onr_opt == 2: #Plot the LO Sweep
                         onrkidpy.plot_lo_sweep()
                     ...
                     if onr_opt == 4:
                         motor.init_test()
                         az_set_pos_req = float(input('What position do 
you want to set the motor (-180.000 to 180.000 degrees)?'))
                         az_set_pos = az_set_pos_req
                         if az_set_pos > 180000 or az_set_pos < -180000:
                             print('This position is outside the limits 
of the Telescope!')
                         else:
                     ...
                     if onr_opt == 5:
                         try:
                             motor.init_test()
                             pos=motor.pos
                             el_set_pos = float(input('What position do 
you want to set the motor (Degrees)?'))
                             motor.setELposition(el_set_pos)
                             print("The EL position is: ", motor.pos)
                         except:
                             motor.set_ao_zero()
                             print ('\033[93m UnboundLocalError: DAQ 
could not be initialized: Check comm port and power supply\033[0m')
