...
class udpcap:
    ...
    def capture_packets(self, N_packets):
        ...
        for i in range(N_packets):
            ...
            if i % 488 == 0:
                print(
                    "{}/{} captured ({:.3f}% Complete)".format(
                        i, N_packets, (N_packets / 488) * 100.0
                    )
                )
        return packets
        ...
        try:
            ...
            while count < nPackets:
                ...
                if count > 0 and count % 488 == 0:
                    print(
                        "{}/{} captured ({:.2f}% Complete)".format(
                            count, nPackets, ((count / nPackets) * 100.0)
                        )
                    )
    ...
    def shortDataCapture(self, fname, nPackets):
        ...
        assert (
            nPackets < 488 * 60
        ), "METHOD NOT INTENDED FOR LONG DATA CAPTURES > 488 packets per second * 60 seconds"
        pData = np.zeros((2052, nPackets))
        try:
            print("capture {} packets".format(nPackets))
            dFile = h5py.File(fname, "w")
            pkts = dFile.create_dataset(
                "PACKETS",
                (2052, nPackets),
                dtype=h5py.h5t.NATIVE_INT32,
                chunks=True,
