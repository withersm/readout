...
def read_chunk_isolate_channel_time(filename, single_channel, time='all', t_index_start=None, t_index_stop=None):
    ...
    #selected_channel.set_index(['t'], inplace=True)
...
if __name__ == "__main__":
    ...
    chunked_data = read_chunk_isolate_channel_time(filename, single_channel, time, t_index_start, t_index_stop)
    print('data has been chunked')
    chunk_to_excel(chunked_data, filename)


# note: from run start to chunk time, this took 05:52 seconds
