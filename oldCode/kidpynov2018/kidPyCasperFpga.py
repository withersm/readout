...
gc = np.loadtxt("./general_config_sam", dtype = "str")
...
ncoeff = 94
coeffs = signal.firwin(ncoeff, cutoff = 0.005, width = 1.0e-4, pass_zero=True, window = "hamming")
#coeffs = np.zeros(94)
def firCoeffs(ncoeff, window_fn="hamming"):
    win_coeffs = signal.get_window(window_fn, ncoeff)
    sinc = signal.firwin(ncoeff, cutoff=0.01*(244.140625/250.0e3), window="rectangular")
    win_coeffs *= sinc
    return win_coeffs
...
def loadFIR(coeffs):    
    ...
    #h = signal.firwin(ncoeff, cutoff = .001, width = 0.0001, pass_zero=True, window = "hamming")
    # coeffs = window.generateCoeffs(94, 488./500.0e3)
    ri.lpf(coeffs)
    return

def load_window(window, ones = True):
    fpga = getFPGA()
    ri = roachInterface(fpga, gc, regs, None)
    #print ri.hanning_2
    coeff32=ri.load_window_coeff(window, ones)
    return coeff32
...
def getFPGA():
    ...
    try:
        fpga = casperfpga.CasperFpga(gc[np.where(gc == 'roach_ppc_ip')[0][0]][1], timeout = 120.)
    except RuntimeError:
...
def testConn(fpga):
    ...
    if not fpga:
        try:
            fpga = casperfpga.CasperFpga(gc[np.where(gc == 'roach_ppc_ip')[0][0]][1], timeout = 3.)
        except RuntimeError:
            print "\nNo connection to ROACH. If booting, wait 30 seconds and retry. Otherwise, check gc config."
    return fpga
...
def vnaSweep(ri, udp, valon):
    ...
    sweep_dir = vna_savepath + '/' + \
       str(int(time.time())) + '-' + time.strftime('%b-%d-%Y-%H-%M-%S')
    os.mkdir(sweep_dir)
    np.save("./last_vna_dir.npy", sweep_dir)
    print sweep_dir
    valon.set_frequency(LO, center_freq/1.0e6)
    span = ri.pos_delta
    print "Sweep Span =", np.round(ri.pos_delta, 2), "Hz"
    start = center_freq*1.0e6 - span#(span/2.)
    stop = center_freq*1.0e6 + span#(span/2.)
    ...
    #nskip = int(udp.buf_max/np.float(buf_size))
    #print nskip
    #udp.configSocket()
    #udp.clearBuffer(nskip)
...
def writeVnaComb(cw = False):                                                             
	...
	if cw:
        ...
        print "Baseband tones =", ri.freq_comb                                     
...
def vnaSweepConsole():
    ...
    print start, stop
    sweep_freqs = np.arange(start, stop + lo_step, lo_step)
    sweep_freqs = np.round(sweep_freqs/lo_step)*lo_step
    if not np.size(ri.freq_comb):
        ri.makeFreqComb()
    np.save(sweep_dir + '/bb_freqs.npy', ri.freq_comb)
    ...
    if not Nchan:
        Nchan = fpga.read_int(regs[np.where(regs == 'read_comb_len_reg')[0][0]][1])
    idx = 0
    # clear out some of the data buffer
    nskip = int(udp.buf_max/np.float(buf_size))
    print nskip
    udp.clearBuffer(nskip)
...
def getLastTargPath():
    return str(np.load("last_targ_dir.npy"))

def getLastVnaPath():
    return str(np.load("last_vna_dir.npy"))

def refitTargFreqsMinMethod():
    fpga = getFPGA()
    ri = roachInterface(fpga, gc, regs, None)
    path = getLastTargPath()
    Is, Qs = openStoredSweep(path)
    bb_targ_freqs = np.load(os.path.join(path, "bb_freqs.npy"))
    sweep_freqs = np.load(os.path.join(path, "sweep_freqs.npy"))
    lo_centerfreq = sweep_freqs[len(sweep_freqs)/2]
    channels = len(bb_targ_freqs)
    ...
    for chan in range(channels):
        ...
        chan_freqs[chan] = (sweep_freqs + bb_targ_freqs[chan])/1.0e6
    ...
    new_freqs = []
    for chan in range(channels):
        min_idx = np.argmin(mags[chan])
        new_freqs.append(chan_freqs[chan][min_idx]*1.0e6 - lo_centerfreq)
    if len(new_freqs) > 0:
        new_freqs = np.roll(new_freqs, - np.argmin(np.abs(new_freqs)))
        # np.save(os.path.join(path, 'bb_targ_freqs.npy'), new_freqs)
    ri.freq_comb = np.roll(new_freqs, - np.argmin(np.abs(new_freqs)) - 1)
    #ri.upconvert = np.sort(((ri.freq_comb + (center_freq)*1.0e6))/1.0e6)
...
def polyfit(x, y, npoints = 1000):
    fit = interp1d(x, y, kind = 'cubic')
    x_new = np.linspace(x[0], x[-1], num = npoints)
    y_new = fit(x_new)
    return x_new, y_new

def fitResFreqs(path):
    Is, Qs = openStoredSweep(path)
    bb_targ_freqs = np.load(os.path.join(path, "bb_freqs.npy"))
    sweep_freqs = np.load(os.path.join(path, "sweep_freqs.npy"))
    nchannels = len(bb_targ_freqs)
    f_res = np.zeros(nchannels)
    for chan in range(nchannels):
        I = Is[:,chan]**2
        Q = Qs[:,chan]**2
        mag = np.sqrt(I**2 + Q**2)
        mag = 20*np.log10(mag/np.max(mag))
        chan_freqs = (sweep_freqs + bb_targ_freqs[chan])/1.0e6
        fit_freqs, mags_new = polyfit(chan_freqs, mag)
        fres = fit_freqs[mags_new == np.min(mags_new)][0]
        x = ((fit_freqs - fres) / fres)

        def magFit(chan_freqs, Qrat, Qr, eps):
            #s21_guess = 1. - ((Qrat)*(1./(1 + 2j*Qr*x)))
            s21_guess = 1. - ((Qrat)*((1. + 1j*eps)/(1. + 1j*eps*(Qrat)))*(1./(1. + ((2j*Qr*x)/(1. + 1j*eps*(Qrat))))))
            mag_guess = np.sqrt(np.real(s21_guess)**2 + np.imag(s21_guess)**2)
            return 20*np.log10(mag_guess)

        popt, pcov = curve_fit(magFit, fit_freqs, mags_new, bounds=([0.01, 1.0e4, 0.], [1.0, 1.0e7, 3.0]), method = 'trf')
        freqs_new, mags_new = polyfit(fit_freqs, magFit(fit_freqs, *popt))
        f_res[chan] = freqs_new[mags_new == np.min(mags_new)][0]
        """
	print
        ...
        print "- F_res =",  f_res[chan], "MHz"
        print
	"""
    return f_res*1.0e6

def refitTargFreqsS21Method(fpga, ri, path):
    path = getLastTargPath()
    new_freqs = fitResFreqs(path)
    new_freqs -= (center_freq*1.0e6)
    print new_freqs
    if len(new_freqs) > 0:
        new_freqs = np.roll(new_freqs, - np.argmin(np.abs(new_freqs)))
        # np.save(os.path.join(path, 'bb_targ_freqs.npy'), new_freqs)
    freq_comb = np.roll(new_freqs, - np.argmin(np.abs(new_freqs)) - 1)
    rf = np.sort(((freq_comb + (center_freq)*1.0e6))/1.0e6)
    print "RF tones =", rf
    if len(freq_comb) > 400:
        ...
        fpga.write_int(regs[np.where(regs == 'fft_shift_reg')[0][0]][1], 2**9 -1)    
        time.sleep(0.1)
    ri.writeQDR(freq_comb)
    np.save("last_freq_comb.npy", freq_comb)
...
def plotPhasePSD(chan, udp, ri, time_interval):
    ...
    plt.ion()
    I, Q = udp.streamChanPhase(chan, time_interval)
    s21 = I + 1j*Q
    phi_rot = np.arctan2(np.mean(Q),np.mean(I))
    s21_rot = s21*np.exp(-1j*phi_rot)
    phases = np.arctan2(np.imag(s21_rot),np.real(s21_rot))
    f, Sii = signal.welch(I, ri.accum_freq)
    f, Sqq = signal.welch(Q, ri.accum_freq)
    f, Spp = signal.welch(phases, ri.accum_freq)
    print Sii
    Spp = 10*np.log10(Spp) 
    Sii = 10*np.log10(Sii) 
    Sqq = 10*np.log10(Sqq) 
    ...
    ax.plot(f, Sii, linewidth = 1, label = 'I', alpha = 0.7)
    ...
    ax.plot(f, Sqq, linewidth = 1, label = 'Q', alpha = 0.7)
    plt.legend(loc = 'upper right')
    plt.grid()
    plt.subplot(3,1,3)
    ax = plt.gca()
    ...
    ax.plot(f, Spp, linewidth = 1, label = 'Phase', alpha = 0.7)
    plt.legend(loc = 'upper right')
    plt.grid()
    return
...
def main_opt(fpga, ri, udp, valon, upload_status):
    ...
    while 1:
        ...
        if opt == 11:
            try:
                path = raw_input("VNA sweep path? ")
		#path = str(np.load("last_vna_dir.npy"))
                print "Sweep path:", path
		print center_freq*1.0e6, lo_step, smoothing_scale, peak_threshold, spacing_threshold
                fitFreqs(path, center_freq*1.0e6, lo_step, smoothing_scale, peak_threshold, spacing_threshold)
		np.save("last_vna_dir.npy", path)
        ...
        if opt == 15:
            ...
            data_path = gc[np.where(gc == 'DIRFILE_SAVEPATH')[0][0]][1]
            sub_dir = raw_input("Insert subfolder name (e.g. single_tone): ")
            try:
                #udp.saveDirfile_chanRange(time_interval)
                udp.saveDirfile_chanRangeIQ(data_path, sub_dir, time_interval, start_chan, end_chan)
