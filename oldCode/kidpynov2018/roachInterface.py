...
class roachInterface(object):
	...
	if transfunc:
        ...
        print("Starting to write to QDR")
        self.fpga.write_int(self.regs[np.where(self.regs == 'dac_reset_reg')[0][0]][1],1)
        ...
        #self.fpga.write(self.regs[np.where(self.regs == 'qdr0_reg')[0][0]][1],I_lut_packed,0)
        #self.fpga.write(self.regs[np.where(self.regs == 'qdr1_reg')[0][0]][1],Q_lut_packed,0)
        print("Finished blindwrite to QDR")
    ...
    def plotArray(self):
        # Generates an array plot with colors scaled by detector responses
        ###############333
        # Plot array with found channels(correspondance table)!
        #################33
        # open correspondence_table.csv
        corr_table = np.loadtxt("correspondance_table.csv",delimiter=',')
	#corr_table = np.loadtxt("generated_correspondance_table.csv",delimiter=',')
        X_table = []
        Y_table = []
        for i in range(len(corr_table)):
          X_table.append(corr_table[i][5])
          Y_table.append(corr_table[i][6])

        #####################
        # Plotting
        #########################
        Responses = np.zeros(len(X_table))*0.0
        Responses_zero = Responses
        #Responses[100] = np.log(200.0)  # fake illuminated pixel

        # Test sinusoidal response
        sinewave = np.sin(np.linspace(0,2*np.pi,10))
	...
	xoffset = 3 # 1000
	yoffset = 3 # 2000
        plt.xlim(min(X_table)-xoffset,max(X_table)+xoffset)
        plt.ylim(min(Y_table)-yoffset,max(Y_table)+yoffset)
        plt.scatter(X_table,Y_table,c=colors,cmap=plt.matplotlib.cm.cool,s=500,linewidth=2,marker = 'h',alpha=0.5)
        plt.tight_layout()
        plt.show(block = False)
        #######################################
        # Get dc baseline for each channel
        I, Q = self.read_accum_snap()
        I = I[2:]
        Q = Q[2:]
        mags_dc =(np.sqrt(I**2 + Q**2))[:len(Responses)]
        count = 0
        stop = 10000
        while(count < stop):
            I, Q = self.read_accum_snap()
            I = I[2:]
            Q = Q[2:]
            #Responses[100] = sinewave[count%len(sinewave)] # test response
            mags =(np.sqrt(I**2 + Q**2))[:len(Responses)]
            plt.scatter(X_table,Y_table,c=mags-mags_dc,cmap=plt.matplotlib.cm.cool,s=500,linewidth=2,marker = 'h',alpha=0.5)
            plt.xlim(min(X_table)-xoffset,max(X_table)+xoffset)
            plt.ylim(min(Y_table)-yoffset,max(Y_table)+yoffset)
            plt.tight_layout()
