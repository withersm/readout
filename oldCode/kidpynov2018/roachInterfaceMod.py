...
class roachInterface(object):
    ...
    def freqCombMod(self, freqs, samp_freq, resolution, random_phase = True, DAC_LUT = True, apply_transfunc = False, **kwargs):
        # Generates a frequency comb for the DAC or DDS look-up-tables. DAC_LUT = True for the DAC LUT. Returns I and Q 
        print("freqCombMod called!")
        freqs = np.round(freqs/self.dac_freq_res)*self.dac_freq_res
	print(freqs)
	amp_full_scale = (2**15 - 1)    
        fft_len = self.LUTbuffer_len
	...
	prompt = raw_input('Load Xfunc,phase,and amp arrays? y/n ')
	if prompt == 'y':
	   T = np.load("x_func.npy")
	   A = np.load("rel_amp.npy")
	   P = np.load("rel_phase.npy")
	else:
	   amp = np.float(raw_input("Relative Amplitude [0:1]: ?"))
	   phase = np.float(raw_input("Relative Phase [rad]: ?"))
	   A = np.array((amp))
	   P = np.array((phase))
	X = np.zeros(fft_len,dtype='complex')
	X[k] = (1.+1j*1.)
	Y, Z = X, X
	Z[k] = X[k]#*T
	x = np.fft.ifft(Z)
	Y[k] = X[k]*np.exp(1j*P)*A#*T # modified spectrum
	w = np.fft.ifft(Y)
	f = x.real + 1j*w.imag

	wMax = np.max(np.abs(f.real))
	yMax = np.max(np.abs(f.imag))
	if wMax > yMax:
	  Max = wMax
	else:
	  Max = yMax
	I = (f.real/Max)*(amp_full_scale)
        Q = (f.imag/Max)*(amp_full_scale)
	# plot for debugging
	plt.figure()
	plt.plot(f.real[0:100],label="I",linewidth=6)
	plt.plot(f.imag[0:100],label="Q",linewidth=4)
	plt.plot(x.real[0:100],label="I orig")
	plt.plot(x.imag[0:100],label="Q orig")
	plt.legend()
	plt.show()
	plt.figure()
    ...
    def pack_luts(self, freqs, transfunc = False, iqBal = False, **kwargs):
    # packs the I and Q look-up-tables into strings of 16-b integers, in preparation to write to the QDR. Returns the string-packed look-up-tables
        if transfunc:
		if iqBal:
			I_dac, Q_dac = self.freqCombMod(freqs, self.dac_samp_freq, self.dac_freq_res, random_phase = True, apply_transfunc = True, **kwargs)
		else:	
			I_dac, Q_dac = self.freqComb(freqs, self.dac_samp_freq, self.dac_freq_res, random_phase = True, apply_transfunc = True, **kwargs)
        else:
		if iqBal:
			I_dac, Q_dac = self.freqCombMod(freqs, self.dac_samp_freq, self.dac_freq_res, random_phase = True)
    ...
    def writeQDR(self, freqs, transfunc = False, iqBal = False, **kwargs):
    # Writes packed LUTs to QDR
	if transfunc:
		if iqBal:
			I_lut_packed, Q_lut_packed = self.pack_luts(freqs, transfunc = True, iqBal=True, **kwargs)
		else:	
			I_lut_packed, Q_lut_packed = self.pack_luts(freqs, transfunc = True, iqBal=False, **kwargs)
	else:
		if iqBal:
			I_lut_packed, Q_lut_packed = self.pack_luts(freqs, transfunc = False, iqBal=True)
		else:	
			I_lut_packed, Q_lut_packed = self.pack_luts(freqs, transfunc = False, iqBal=False)
        self.fpga.write_int(self.regs[np.where(self.regs == 'dac_reset_reg')[0][0]][1],1)
    ...
    def plotWaterfall(self):
		...
		lines = []
		colors = ['w','g','r','b','m']
		labels = ['chan 174','chan 172','chan 168','chan 246','chan 247']
		for i in range(len(data)):
		    # Small reduction of the X extents to get a cheap perspective effect
		    xscale = 1 - i / 400.
		    # Same for linewidth (thicker strokes on bottom)
		    lw = 1.5 - i / 100.0
		    line, = ax.plot(xscale * X, i + data[i], color=colors[i],label = labels[i], lw=lw)
		    ax.legend()
		    lines.append(line)
